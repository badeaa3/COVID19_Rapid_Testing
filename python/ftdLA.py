...
tests_tot = np.array(DATA['tests']['tot_tests'])[DATA['tests']['date'].index(sdate):DATA['tests']['date'].index(edate)] / pop
...
def resid(par,y=[],tot=[],H=[],E=[],Phi=[],window=7):
    Sn, Qun, In, Dn, Hn, Rn, QRn, En, totn, totaln = funct(y,par)
    residual = []
    for r in range(window):
        residual += list([10*(tot[r] - totn)/(tot[r]+10**-50), 10*(H[r] - Hn)/(H[r]+10**-50), 10*(E[r] - En)/(E[r]+10**-50), (0.02 - (Hn)/(In+Dn+Hn+10**-50))/0.02])
...
def make_fig_dates():
    ...
    '#000000','#1f77b4', '#ff7f0e', '#ffbb78', '#2ca02c', '#98df8a',
...
def make_fig():
    f = plt.figure(figsize=(10,10))
    ax = f.add_subplot(111)
    ax.set_xlabel("Frequency (1 / Days Between Tests)",fontsize=16)
    ax.set_ylabel("Percent of Population (%)",fontsize=16)
    ax.tick_params(labelsize=15, direction='out', length=6, width=2)
    #ax.ticklabel_format(style='sci', axis='y', scilimits=(0,0),useMathText=True)
...
def get_states(freq=1/7,sens=0.8,spec=0.9, wind=1,window=7,PD=None):
